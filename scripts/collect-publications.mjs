...
await fs.writeFile(path.join(dataDir,"artigos.csv"),["openalex_id,doi,titulo,ano,periodico,source_id,issn,url,acesso_aberto,autores,docentes_ppge,linhas",...artigos.map(a=>[a.openalexId,a.doi,a.titulo,a.ano,a.periodico,a.sourceId,a.issn,a.url,a.acessoAberto,a.autores.join("; "),a.docentes.map(d=>d.nome).join("; "),a.linhas.join("; ")].map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(","))].join("\n")+"\n");
